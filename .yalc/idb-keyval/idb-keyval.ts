...
export function compareAndSwap<Type>(key: IDBValidKey, value: any, compare: (a: any) => boolean, store = getDefaultStore()): Promise<Type> {
  let req: IDBRequest;
  let result: any;
  let error: string;
  return store._dbp.then(db => new Promise<void>((resolve, reject) => {
    ...
    const _store = transaction.objectStore(store.storeName);
    let req = _store.get(key)
    req.onsuccess = () => {
        result = req.result;
        if (!compare(result)) {
          error = "COMPARE_ERROR"
